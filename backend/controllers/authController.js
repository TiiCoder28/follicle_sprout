const { signupSchema, loginSchema, acceptCodeSchema, changePasswordSchema } = require("../middlewares/validator");
...
exports.sendForgotPasswordCode = async (req, res) => {
    ...
    try {
        ...
        if (info.accepted && info.accepted.includes(existingUser.email)) {
            ...
            existingUser.passwordCodeValidation = Date.now();
            await existingUser.save();
...
exports.verifyPasswordCode = async (req, res) => {
    ...
    try {
        ...
        // Fetch the user and include the necessary fields
        const existingUser = await user.findOne({ email }).select("+passwordCode +passwordCodeValidation");
        console.log('Existing User:', existingUser); // Log user data for debugging
        ...
        if (!existingUser.passwordCode) {
            return res.status(400).json({ 
                success: false, 
                message: 'Reset code missing. Please request a new code.' 
            });
        }
        // Check if the verification code has expired
        const expirationTime = 5 * 60 * 1000; // 5 minutes in milliseconds
        const timeDifference = Date.now() - existingUser.passwordCodeValidation;
        ...
        if (providedCode.toString() === existingUser.passwordCode.toString()) {
            
            existingUser.passwordCode = undefined; // Clear the code
            existingUser.passwordCodeValidation = undefined; // Clear the timestamp
            existingUser.password = undefined;
            await existingUser.save();

            return res.status(200).json({ success: true, message: 'Your password has been reset. Enter a new password' });
        }
